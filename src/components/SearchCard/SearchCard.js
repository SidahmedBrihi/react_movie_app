...
const SearchCard = (props) => {
  ...
  return (
    <div className='search-movie' id={props.data.id} >
        <img src={`https://image.tmdb.org/t/p/w780${props.data.poster_path}`} alt=''/>
        ...
        <div className="content">
          ...
          <Link to={props.data.media_type === 'movie' ? `/movie/${props.data.id}`:
           (props.data.media_type === 'tv' ? `/tv/${props.data.id}` : console.log() ) }
            className="view" id={props.data.id} ><span><AiFillEye /></span> view </Link>
          <br />
